...
from data_utils import create_indexes, load_examples
...
def _answer(llm_type: OpenAI, question: str, sql_query: str, sql_result: Optional[str]) -> str:
    prompt = ANSWER_TEMPLATE.format(question=question, sql_query=sql_query, sql_result=sql_result)
    response = llm_type.chat([ChatMessage(role=MessageRole.USER, content=prompt)])

    return response.message.content or ""


def _match(llm_type: OpenAI, question: str, reference_answer: str, hypothesis_answer: str) -> bool:
...
def _get_answers(llm_type: OpenAI, indexes_dict: Dict[str, SQLStructStoreIndex], db_names: List[str], 
                 sql_queries: List[str], examples: List[dict], output_filename: 
                 str, use_cache: bool) -> List[dict]:
    ...
    results = []
    for db_name, sql_query, example in tqdm(list(zip(db_names, sql_queries, examples)),
        ...
        try:
            query_engine = indexes_dict[db_name].as_query_engine(query_mode=SQLQueryMode.SQL)
            ...
            result["answer"] = _answer(llm_type, question, sql_query, resp.response)
...
def _match_answers(llm_type: OpenAI, gold_results: List[dict], pred_results: List[dict], 
                   examples: List[dict], output_filename: str) -> float:
    results = []
    for gold, pred, example in tqdm(list(zip(gold_results, pred_results, examples)), 
        ...
        if pred["answer"] is None or gold["answer"] is None:
            ...
            answer_match = _match(llm_type, example["question"], gold["answer"], pred["answer"])
    ...
    valid_results = [e for e in results if e["exec_match"] is not None and e["answer_match"] is not None
    ]
    answer_accuracy = sum([e["exec_match"] or e["answer_match"] for e in valid_results]) / float(len(valid_results))
...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.predict_dir, "train_pred.sql"), encoding="utf-8") as f:
        train_pred_sqls = f.readlines()

    with open(os.path.join(args.predict_dir, "dev_pred.sql"), encoding="utf-8") as f:
    ...
    with open(os.path.join(args.spider_dir, "train_gold.sql"), encoding="utf-8") as f:
        for line in f.readlines():
            line_tokens = line.strip().split("\t")
            train_gold_sqls.append(line_tokens[0])
            train_dbs.append(line_tokens[1])

    with open(os.path.join(args.spider_dir, "dev_gold.sql"), encoding="utf-8") as f:
        for line in f.readlines():
            line_tokens = line.strip().split("\t")
