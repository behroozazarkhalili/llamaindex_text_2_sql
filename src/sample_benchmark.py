...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.input, "train_spider.json"), encoding="utf-8") as f:
        train_spider = json.load(f)

    with open(os.path.join(args.input, "train_others.json"), encoding="utf-8") as f:
        train_others = json.load(f)

    with open(os.path.join(args.input, "dev.json"), encoding="utf-8") as f:
        dev = json.load(f)
    ...
    train_spider_indices = random.choices(train_spider_indices, k=int(args.sample_factor * len(train_spider_indices)))
    train_others_indices = random.choices(train_others_indices, k=int(args.sample_factor * len(train_others_indices)))
    dev_indices = random.choices(dev_indices, k=int(args.sample_factor * len(dev_indices)))
    ...
    with open(os.path.join(args.output, "train_spider.json"), "w", encoding="utf-8") as f:
        json.dump([train_spider[i] for i in train_spider_indices], f, indent=2)

    with open(os.path.join(args.output, "train_others.json"), "w", encoding="utf-8") as f:
        json.dump([train_others[i] for i in train_others_indices], f, indent=2)
        
    with open(os.path.join(args.output, "dev.json"), "w", encoding="utf-8") as f:
        json.dump([dev[i] for i in dev_indices], f, indent=2)

    # Write the sql files to the output directory.
    with open(os.path.join(args.output, "train_gold.sql"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(args.output, "dev_gold.sql"), "w", encoding="utf-8") as f:
